...
class PuzzleEngine {
  ...
  generatePieces() {
    ...
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        const topTab =
          r === 0 ? 0 : -this.pieces[(r - 1) * cols + c].tabs.bottom;
        const leftTab =
          c === 0 ? 0 : -this.pieces[r * cols + (c - 1)].tabs.right;
        const bottomTab =
          this.pieceShape === "grid" || r === rows - 1
            ? 0
            : Math.random() > 0.5
              ? 1
              : -1;
        const rightTab =
          this.pieceShape === "grid" || c === cols - 1
            ? 0
            : Math.random() > 0.5
              ? 1
              : -1;
  ...
  drawEdge(ctx, x1, y1, x2, y2, tab, size) {
    ...
    // Draw bulb Left half
    ctx.bezierCurveTo(
      midX - nx * size * 0.5 - ex * 0.2,
      midY - ny * size * 0.5 - ey * 0.2,
      midX - nx * size * 1.2 + ex,
      midY - ny * size * 1.2 + ey,
      midX + ex,
      midY + ey,
  ...
  pickGroup(pos) {
    ...
    if (pickedPiece) {
      this.selectedGroup = this.pieces.filter(
        (p) => p.groupId === pickedPiece.groupId,
      );
  ...
  releaseGroup() {
    ...
    for (const p of this.selectedGroup) {
      const neighbors = this.pieces.filter(
        (n) =>
          n.groupId !== p.groupId &&
          ((n.col === p.col - 1 && n.row === p.row) ||
            (n.col === p.col + 1 && n.row === p.row) ||
            (n.col === p.col && n.row === p.row - 1) ||
            (n.col === p.col && n.row === p.row + 1)),
      );
      for (const n of neighbors) {
        ...
        if (
          Math.hypot(p.x - expX, p.y - expY) < 30 &&
          Math.abs(p.rotation - n.rotation) < 0.2
        ) {
          const sx = expX - p.x;
          const sy = expY - p.y;
          const rot = n.rotation;
          const gid = n.groupId;
          this.selectedGroup.forEach((gp) => {
            gp.x += sx;
            gp.y += sy;
            gp.rotation = rot;
            gp.groupId = gid;
    ...
    if (!snapped) {
      for (const p of this.selectedGroup) {
        ...
        if (
          Math.hypot(p.x - p.targetX, p.y - p.targetY) < 30 &&
          (rotDiff < 0.2 || rotDiff > Math.PI * 2 - 0.2)
        ) {
  ...
  checkSolve() {
    const allOne = this.pieces.every(
      (p) => p.groupId === this.pieces[0].groupId,
    );
    if (
      allOne &&
      Math.hypot(
        this.pieces[0].x - this.pieces[0].targetX,
        this.pieces[0].y - this.pieces[0].targetY,
      ) < 5
    ) {
      this.forceSolve();
    }
  ...
  getHintCandidatePieces() {
    return this.pieces.filter(
      (p) => !this.isPieceCorrect(p) && !this.hintedPieceIds.has(p.id),
    );
  }
  ...
  getFinalCanvas(frameStyle = "none") {
    ...
    // Draw crisp black jigsaw seams with subtle 3D depth
    this.pieces.forEach((p) => {
      const x = padding + p.col * sW;
      const y = padding + p.row * sH;
      ctx.save();
